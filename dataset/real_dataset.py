...
class RealFireDataset(Dataset):
    ...
    def __init__(self, cfg):
        ...
        self.root_dir = cfg['real_root_dir']
        ...
        for rd in self.root_dir:
            for root, dirs, files in os.walk(os.path.join(rd, self.fire_dir)):
                if root == os.path.join(rd, self.fire_dir):
                    ...
                    sorted_files = sorted(files, key=lambda x: datetime.strptime(x.split('_')[1], '%Y%m%d'))
                    length = len(sorted_files)
                    sequences = self.extract_sequences(length, self.sample)
                    for s in sequences:
                        timestamps = [datetime.strptime(sorted_files[i].split('_')[1], '%Y%m%d') for i in s]
                        self.time_steps.append(self.normalize_timestamps(timestamps))
                        ind = int(len(s) / 2)
                        ...
                        for ind, line_sequence in enumerate(input_sequence):
                            ...
                            key = f'{rd}_{os.path.basename(root)}_{line_sequence}'
    ...
    def __getitem__(self, index): 
        ...
        input_squence = [self.process_image(file) for file in fire_path]
        input_squence = torch.cat(input_squence, dim=0).float()

        output_squence = [self.process_image(file) for file in target_path]
        output_squence = torch.cat(output_squence, dim=0).float()

        sate_squence = [self.process_image(file) for file in sate_path]
                ...
                for filename in os.listdir(wxs_dir):
                    ...
                    with open(file_path, 'r', encoding='utf-8') as file:
                        ...
                        for line_number, line_content in enumerate(lines, 1):
                            key = f"{rd}_{filename}_{line_number}"
